...
def generate():
    try:
        ...
        if not uploaded_files:
            flash("Please add your GSTR-1, GSTR-2B Summary, and GSTR-3B files before generating.")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if not by_key:
                flash("Couldn't recognize any GSTR-1, GSTR-2B Summary, or GSTR-3B files in what you uploaded. "
            ...
            # Group by period, keep only periods with exactly one file per return type
            periods = sorted(set(k[1] for k in by_key.keys()))
            notes = []
            gstr3b_records, gstr1_records, gstr2b_records = [], [], []
            cmp_3b1, cmp_3b2b = [], []

            for period in periods:
                r3b_files = by_key.get((gstin, period, "gstr3b"), [])
                r1_files = by_key.get((gstin, period, "gstr1"), [])
                r2b_files = by_key.get((gstin, period, "gstr2b"), [])

                multi_part = [(t, fs) for t, fs in
                              [("GSTR-1", r1_files), ("GSTR-2B", r2b_files), ("GSTR-3B", r3b_files)]
                              if len(fs) > 1]
                if multi_part:
                    parts_desc = ", ".join(f"{t} ({len(fs)} files)" for t, fs in multi_part)
                    notes.append(f"{period}: skipped -- multiple files detected for {parts_desc}. "
                                 f"This may be a large-data split file this tool doesn't yet merge -- "
                                 f"only complete, single-file periods are included below.")
                    continue

                missing = [t for t, fs in [("GSTR-1", r1_files), ("GSTR-2B", r2b_files), ("GSTR-3B", r3b_files)]
                           if not fs]
                if missing:
