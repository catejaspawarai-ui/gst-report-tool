    ...
    for gstin in gstins:
        ...
        for section in _3B_VS_1_SECTIONS:
            ...
            for t in section["types"]:
                ...
                # only computed when BOTH sides genuinely exist for that period;
                # a period with only one side present shows blank, not a false
                # variance equal to whichever side happens to be missing.
                diff_vals = []
                any_data = False
                for i, p in enumerate(periods):
                    r3b_present = p in r3b_periods
                    r1_present = p in r1_periods
                    if not (r3b_present and r1_present):
